...
<body>
	...
	<div>
		<c:if test="${page.prev}">
		  <span>[ <a href="searchListPage?num=${page.startPageNum - 1}&searchOption=${searchOption}&keyword=${keyword}">이전</a> ]</span>
		</c:if>	
		<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="num">
			<span>
 				<c:if test="${select != num}"><a href="searchListPage?num=${num}&searchOption=${searchOption}&keyword=${keyword}">${num}</a></c:if>    
  				<c:if test="${select == num}"><b>${num}</b></c:if>
  			</span>
		</c:forEach>
		<c:if test="${page.next}">
		 <span>[ <a href="searchListPage?num=${page.endPageNum + 1}&searchOption=${searchOption}&keyword=${keyword}">다음</a> ]</span>
		</c:if>
	</div>
 	<a href="move">글 쓰기</a>
 	<a href="listPage">검색 초기화</a>  
	</center>
